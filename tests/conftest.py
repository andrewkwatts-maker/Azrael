"""Shared fixtures for azrael tests.

Strategy: patch azrael._query._db._base with a MagicMock whose fetchone/fetchall/execute
side-effects delegate to a real in-memory SQLite connection containing the full schema
and a small set of known entities.
"""
import json
import sqlite3
...
def make_test_db(entities: list[dict]) -> sqlite3.Connection:
    """Create an in-memory SQLite DB with the full azrael schema and insert entities."""
...
def patch_base(db):
    ...
    # Also reset graph/corpus lazy singletons so they use our mock conn
