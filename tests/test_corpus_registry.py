...
from azrael._corpus_registry import MYTHOLOGY_CORPUSES  # noqa: E402


def test_registry_is_list():
    """MYTHOLOGY_CORPUSES is a non-empty list with more than 5 entries."""
    assert isinstance(MYTHOLOGY_CORPUSES, list)
    assert len(MYTHOLOGY_CORPUSES) > 5


def test_registry_structure():
    """Every entry has the required keys with valid source_type values."""
    valid_source_types = {"gutenberg", "url", "git"}
    for entry in MYTHOLOGY_CORPUSES:
...
def test_registry_ids_are_unique():
    ...
    ids = [entry["id"] for entry in MYTHOLOGY_CORPUSES]
